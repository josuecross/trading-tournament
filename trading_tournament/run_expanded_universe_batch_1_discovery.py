...
OUTPUT_DIR = Path("evidence") / "parallel_research_discovery" / "expanded_universe_batch_1" / "latest"
BATCH_ID = "expanded_universe_batch_1_regional_international_lowvol"
DATA_HISTORY_MODE = base.DATA_HISTORY_MODE
EXPANSION_SYMBOLS = {"EWJ", "EWU", "EWG", "EWY", "INDA", "EFAV", "EEMV", "SCHG"}
DEFERRED_SYMBOLS = {"IEFA", "VEA", "VWO", "IWF", "IWD", "SCHV", "ACWV"}
FORBIDDEN_SYMBOLS = {"DBC"} | DEFERRED_SYMBOLS
BENCHMARK_IDS = [base.VM_ID, base.DSR_ID, base.SPY_200D_ID, "SPY_buy_hold", "QQQ_buy_hold", "BIL_cash_proxy"]
NEXT_ACTIONS = {
    "promotion": "create_promotion_review_for_best_expanded_universe_batch_1_candidate",
    "diversifier": "review_best_expanded_universe_diversifier_candidate_or_continue_discovery",
    "none": "continue_next_expanded_universe_discovery_batch_or_pause",
    "data_issue": "repair_expanded_universe_batch_1_data_issue",
}


def specs() -> list[dict[str, Any]]:
    return [
        {"strategy_id": "rim_regional_top2_momentum_bil_v1", "family_group": "regional_international_momentum", "symbols": ["EWJ", "EWU", "EWG", "EWY", "INDA", "EFA", "EEM", "BIL"], "rule": "regional_topn", "n": 2},
        {"strategy_id": "rim_regional_top3_momentum_bil_v1", "family_group": "regional_international_momentum", "symbols": ["EWJ", "EWU", "EWG", "EWY", "INDA", "EFA", "EEM", "BIL"], "rule": "regional_topn", "n": 3},
        {"strategy_id": "rim_regional_momentum_with_spy_gate_v1", "family_group": "regional_international_momentum", "symbols": ["SPY", "EWJ", "EWU", "EWG", "EWY", "INDA", "EFA", "EEM", "BIL"], "rule": "regional_spy_gate"},
        {"strategy_id": "ilv_international_lowvol_defensive_v1", "family_group": "international_lowvol_defensive", "symbols": ["EFAV", "EEMV", "USMV", "SPLV", "BIL"], "rule": "lowvol_top2"},
        {"strategy_id": "ilv_international_lowvol_60_40_bil_v1", "family_group": "international_lowvol_defensive", "symbols": ["EFAV", "EEMV", "USMV", "SPLV", "BIL"], "rule": "lowvol_60_40"},
        {"strategy_id": "ilv_regional_lowvol_blend_v1", "family_group": "international_lowvol_defensive", "symbols": ["EFAV", "EEMV", "EWJ", "EWU", "EWG", "EWY", "INDA", "BIL"], "rule": "regional_lowvol_blend"},
        {"strategy_id": "rgb_regional_gold_bond_top3_v1", "family_group": "regional_gold_bond_defensive_rotation", "symbols": ["EWJ", "EWU", "EWG", "EWY", "INDA", "GLD", "IEF", "TLT", "AGG", "BIL"], "rule": "regional_gold_bond_top3"},
        {"strategy_id": "rgb_regional_growth_defensive_70_30_v1", "family_group": "regional_gold_bond_defensive_rotation", "symbols": ["EWJ", "EWU", "EWG", "EWY", "INDA", "EFA", "EEM", "GLD", "IEF", "TLT", "AGG", "BIL"], "rule": "regional_growth_defensive_70_30"},
        {"strategy_id": "rgb_regional_canary_defensive_v1", "family_group": "regional_gold_bond_defensive_rotation", "symbols": ["SPY", "EFA", "EWJ", "EWU", "EWG", "EWY", "INDA", "EEM", "GLD", "IEF", "TLT", "AGG", "BIL"], "rule": "regional_canary_defensive"},
        {"strategy_id": "sgc_schg_qqq_mtum_quality_cash_v1", "family_group": "selective_growth_comparator", "symbols": ["SCHG", "QQQ", "MTUM", "QUAL", "SPY", "BIL"], "rule": "schg_growth_cash"},
        {"strategy_id": "sgc_schg_regional_mix_v1", "family_group": "selective_growth_comparator", "symbols": ["SCHG", "QQQ", "MTUM", "QUAL", "EWJ", "EWU", "EWG", "EWY", "INDA", "BIL"], "rule": "schg_regional_mix"},
        {"strategy_id": "benchmark_regional_equal_weight_trend_v1", "family_group": "benchmark_sanity_rows", "symbols": ["EWJ", "EWU", "EWG", "EWY", "INDA", "BIL"], "rule": "benchmark_regional_equal_weight"},
        {"strategy_id": "benchmark_international_lowvol_equal_weight_v1", "family_group": "benchmark_sanity_rows", "symbols": ["EFAV", "EEMV", "BIL"], "rule": "benchmark_lowvol_equal_weight"},
    ]
...
def required_symbols() -> list[str]:
    symbols = {"SPY", "QQQ", "BIL", "SPLV", "USMV", "QUAL", "MTUM", "EFA", "EEM", "XLK", "XLF", "XLE", "XLV", "XLY", "XLP", "XLU", "XLI", "XLB", "XLC"}
    for spec in specs():
...
def state_mismatches(root: Path, registry: dict[str, Any]) -> list[str]:
    ...
    symbol_rows = {str(row.get("symbol", "")).upper(): row for row in load_yaml(root / base.SYMBOL_MAP_PATH).get("symbols", [])}
    for symbol in sorted(EXPANSION_SYMBOLS):
        row = symbol_rows.get(symbol, {})
        if row.get("approved_status") != "approved_cache_ready" or row.get("cache_ready") is not True or row.get("qa_status") != "passed":
            mismatches.append(f"{symbol} is not approved cache-ready")
    for symbol in sorted(DEFERRED_SYMBOLS):
        if symbol_rows.get(symbol, {}).get("approved_status") in {"approved_pending_cache_bootstrap", "approved_cache_ready"}:
            mismatches.append(f"deferred symbol {symbol} is approved")
    bootstrap_manifest = root / "evidence" / "approved_expansion_cache_bootstrap" / "latest" / "approved_expansion_cache_manifest.json"
    if not bootstrap_manifest.exists():
        mismatches.append("approved expansion cache bootstrap manifest missing")
    else:
        manifest = json.loads(bootstrap_manifest.read_text(encoding="utf-8"))
        if manifest.get("next_action") != "run_expanded_universe_discovery_batch":
            mismatches.append("current next action is not run_expanded_universe_discovery_batch")
        if sorted(manifest.get("symbols_failed", [])):
            mismatches.append("approved expansion cache bootstrap has failed symbols")
    return mismatches
...
def ranked(close: pd.DataFrame, symbols: list[str], t: int, require_eligible: bool = True) -> list[str]:
    scored: list[tuple[str, float]] = []
    for symbol in symbols:
        if require_eligible and symbol != "BIL" and not base.eligible(close, symbol, t):
            continue
...
def allocate_top(weights: dict[str, float], close: pd.DataFrame, symbols: list[str], t: int, sleeve: float, n: int) -> None:
    picks = ranked(close, symbols, t)[:n]
    for symbol in picks:
        add(weights, symbol, sleeve / n)
    add(weights, "BIL", sleeve * (n - len(picks)) / n)


def best(close: pd.DataFrame, symbols: list[str], t: int, require_eligible: bool = True) -> str:
    pool = ranked(close, symbols, t, require_eligible=require_eligible)
...
def strategy_weights(close: pd.DataFrame, strategy_id: str, t: int, specs_by_id: dict[str, dict[str, Any]]) -> dict[str, float]:
    ...
    regional = ["EWJ", "EWU", "EWG", "EWY", "INDA", "EFA", "EEM"]
    regional_only = ["EWJ", "EWU", "EWG", "EWY", "INDA"]
    defensive = ["GLD", "IEF", "TLT", "AGG", "BIL"]
    defensive_no_bil = ["GLD", "IEF", "TLT", "AGG"]
    if rule == "regional_topn":
        allocate_top(weights, close, regional, t, 1.0, int(spec["n"]))
    elif rule == "regional_spy_gate":
        if base.eligible(close, "SPY", t):
            allocate_top(weights, close, regional, t, 1.0, 2)
        else:
            add(weights, "BIL", 1.0)
    elif rule == "lowvol_top2":
        allocate_top(weights, close, ["EFAV", "EEMV", "USMV", "SPLV"], t, 1.0, 2)
    elif rule == "lowvol_60_40":
        allocate_top(weights, close, ["EFAV", "EEMV", "USMV", "SPLV"], t, 0.60, 2)
        add(weights, "BIL", 0.40)
    elif rule == "regional_lowvol_blend":
        add(weights, best(close, ["EFAV", "EEMV"], t), 0.50)
        add(weights, best(close, regional_only, t), 0.30)
        add(weights, "BIL", 0.20)
    elif rule == "regional_gold_bond_top3":
        allocate_top(weights, close, regional_only + defensive_no_bil, t, 1.0, 3)
    elif rule == "regional_growth_defensive_70_30":
        allocate_top(weights, close, regional, t, 0.70, 2)
        add(weights, best(close, defensive, t, require_eligible=False), 0.30)
    elif rule == "regional_canary_defensive":
        if base.eligible(close, "SPY", t) and base.eligible(close, "EFA", t):
            allocate_top(weights, close, regional, t, 0.70, 2)
            add(weights, best(close, defensive, t, require_eligible=False), 0.30)
        else:
            add(weights, best(close, defensive_no_bil, t), 0.40)
            add(weights, "BIL", 0.60)
    elif rule == "schg_growth_cash":
        allocate_top(weights, close, ["SCHG", "QQQ", "MTUM", "QUAL", "SPY"], t, 0.70, 2)
        add(weights, "BIL", 0.30)
    elif rule == "schg_regional_mix":
        add(weights, best(close, ["SCHG", "QQQ", "MTUM", "QUAL"], t), 0.50)
        add(weights, best(close, regional_only, t), 0.30)
        add(weights, "BIL", 0.20)
    elif rule == "benchmark_regional_equal_weight":
        for symbol in regional_only:
            add(weights, symbol if base.eligible(close, symbol, t) else "BIL", 0.20)
    elif rule == "benchmark_lowvol_equal_weight":
        for symbol in ["EFAV", "EEMV"]:
            add(weights, symbol if base.eligible(close, symbol, t) else "BIL", 0.50)
...
def classify(summary: dict[str, Any], stress: dict[str, Any], deltas: dict[str, Any], correlations: dict[str, Any], family: str) -> tuple[str, str]:
    ...
    if summary["target_300_before_stop_rate"] < 0.2 or summary["median_final_equity"] < 3150 or weak_refs:
        return "too_slow_for_profit_goal", "target/profit profile too weak or benchmark-weaker"
...
def create_packet(directory: Path) -> Path:
    packet = directory / "expanded_universe_batch_1_packet.zip"
    if packet.exists():
        packet.unlink()
...
def write_outputs(root: Path, payload: dict[str, Any], consistency: dict[str, Any]) -> dict[str, str]:
    ...
    write_csv(output / "expanded_universe_batch_1_results.csv", payload["rows"], RESULT_FIELDS)
    write_csv(output / "expanded_universe_batch_1_family_summary.csv", payload["family_rows"], ["family_group", "row_count", "best_strategy_id", "best_180d_median_final_equity", "promotion_candidates", "watchlist_candidates"])
    write_csv(output / "expanded_universe_batch_1_benchmark_delta.csv", payload["benchmark_rows"], ["strategy_id", "benchmark_id", "strategy_180d_median_final_equity", "benchmark_180d_median_final_equity", "delta", "correlation", "comparison_status"])
    write_csv(output / "expanded_universe_batch_1_decision_log.csv", payload["decision_rows"], ["strategy_id", "family_group", "decision", "decision_reason", "next_allowed_step"])
    write_csv(output / "expanded_universe_batch_1_watchlist.csv", [r for r in payload["rows"] if "watchlist" in r["decision"]], RESULT_FIELDS)
    write_csv(output / "expanded_universe_batch_1_promotion_candidates.csv", [r for r in payload["rows"] if r["decision"] == "promotion_review_candidate"], RESULT_FIELDS)
    write_csv(output / "expanded_universe_batch_1_rejects.csv", [r for r in payload["rows"] if r["decision"] in {"reject", "too_risky", "too_slow_for_profit_goal", "duplicate_or_near_duplicate", "evidence_missing", "needs_benchmark_delta_review"}], RESULT_FIELDS)
    (output / "expanded_universe_batch_1_next_action.md").write_text(f"# Next Action\n\n`{payload['next_action']}`\n\nDo not run candidate_exhaustive or any paper-forward/broker/real-money workflow directly from this discovery batch.\n", encoding="utf-8")
    summary = ["# Expanded Universe Batch 1", "", f"Created at UTC: {now_utc()}", f"Batch: `{BATCH_ID}`", f"Rows tested: {len(payload['rows'])}", f"Best row overall: `{payload['best_row']}`", f"Promotion candidates: {sum(1 for r in payload['rows'] if r['decision'] == 'promotion_review_candidate')}", f"Diversifier watchlist candidates: {sum(1 for r in payload['rows'] if r['decision'] == 'diversifier_watchlist_candidate')}", f"Next action: `{payload['next_action']}`", "", "Discovery only; no candidate validation, paper-forward action, broker/live-order, provider download, or real-money recommendation."]
    (output / "expanded_universe_batch_1_summary.md").write_text("\n".join(summary) + "\n", encoding="utf-8")
    ...
    write_json(output / "expanded_universe_batch_1_manifest.json", manifest)
    write_json(output / "expanded_universe_batch_1_consistency_check.json", consistency)
    packet = create_packet(output)
    return {"output_dir": str(output), "packet": str(packet)}


def run_expanded_batch_1(root: Path = ROOT, strict_state: bool = True) -> dict[str, Any]:
    registry = load_yaml(root / base.REGISTRY_PATH)
    ...
    consistency = {"approved_cache_ready_symbols_only": True, "deferred_symbols_rejected": True, "fixed_rules_predefined": True, "parallel_discovery_completed": True, "cache_used": payload["diagnostics_available"] and not payload["missing_symbols"], "no_candidate_exhaustive_run": True, "no_paper_forward_review": True, "no_paper_forward_activation": True, "no_paper_forward_checkpoint": True, "no_active_observation_mutation": True, "no_broker_path_added": True, "no_live_order_path_added": True, "no_real_money_recommendation": True, "benchmark_unavailable_not_zero_filled": any(r["benchmark_id"] == "active_combo" and r["delta"] == "unavailable" for r in payload["benchmark_rows"]), "risk_buffer_gate_applied": all(r["decision"] != "promotion_review_candidate" or float(r["risk_buffer_vs_minus_600"]) >= 50 for r in payload["rows"]), "needs_benchmark_delta_review_only_for_missing_deltas": all(row["decision"] != "needs_benchmark_delta_review" or "unavailable" in key_deltas_by_strategy.get(row["strategy_id"], []) for row in payload["rows"]), "next_action_explicit": bool(payload["next_action"])}
...
def main() -> None:
    print(json.dumps(run_expanded_batch_1(ROOT, strict_state=True), indent=2, sort_keys=True))
