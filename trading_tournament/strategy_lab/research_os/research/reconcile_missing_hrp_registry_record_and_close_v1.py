...
BLOCKED_CLOSURE_DIR = ROOT / "evidence" / "lifecycle" / "close_hrp_after_validation_v1" / "latest"
EXPLORATION_DIR = ROOT / "evidence" / "research_recovery" / "fast_source_library_remaining_candidates_batch_v4" / "latest"
...
ARCHITECTURE = "hierarchical_risk_based_multi_asset_allocation"
SOURCE_LINEAGE = "strategy_source_library_refresh_v1__lopez_de_prado_hrp"
...
STRATEGY_NEXT_ACTION = "do_not_retest_exact_hrp_five_asset_configuration"
PROJECT_NEXT_ACTION_SUCCESS = "refresh_strategy_source_library_v2"
PROJECT_NEXT_ACTION_BLOCKED = "direction_owner_review_hrp_registry_alias_conflict_v1"
...
FINGERPRINT_SCHEMA_VERSION = "hrp_exact_config_fingerprint_v1"
FROZEN_PARAMETERS = {
...
def normalize_parameters(value: Any) -> dict[str, Any]:
    if isinstance(value, str):
        ...
        parsed = {}
    aliases = {
        "hrp_lookback_trading_days": "lookback_trading_days",
    ...
    if normalized.get("warmup_rule") == "equal_weight":
        normalized["warmup_rule"] = "equal_weights_before_252_observations"
    if normalized.get("linkage_method") == "single":
        normalized["linkage_method"] = "single"
...
def record_fingerprint_payload(record: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "lookback_trading_days": int(params.get("lookback_trading_days", 0) or 0),
        "covariance_estimator": params.get("covariance_estimator", ""),
        "distance_formula": params.get("distance_formula", ""),
...
def target_registry_record() -> dict[str, Any]:
    return {
        ...
        "closure_scope": "exact_five_etf_252d_sample_cov_single_linkage_monthly_next_session_close_configuration_only",
        "data_source": "existing_adjusted_etf_cache",
        "implementation_status": "archived",
        "evidence_source": "hrp_incremental_diversifier_validation_v1",
        "latest_evidence_path": rel(VALIDATION_DIR),
        "latest_known_result_summary": "Validation failed: IEF or BIL economically replicated HRP contribution.",
        "role": "closed_exact_configuration",
        "rules_frozen": True,
        "risk_framework_status": "not_paper_demo_eligible",
        "risk_budget_status": "closed_no_incremental_value",
        "promotion_decision": "do_not_promote",
        "promotion_review_required": False,
        "promotion_reason": "Closed exact HRP configuration after validation failed with benchmark-like contribution.",
        "promotion_blockers": "validation_failed;benchmark_like_behavior;not_paper_demo_eligible;no_real_money_authorization",
        "promotion_requirements": "A materially distinct new HRP configuration and direction-owner evidence would be required before any future review.",
        "demotion_or_kill_criteria": "Exact tested configuration is already closed after validation failure.",
        "notes": "Retrospective registry reconciliation for the exact tested five-asset HRP configuration only; family-wide HRP research is not universally closed by this record.",
        "instrument_lane": "ETF",
        "evidence_tier": "blocked",
        "primary_failure_mode": "benchmark_like_behavior",
        "duplication_risk": "exact_configuration_no_incremental_value",
...
def inspect_registry(registry: dict[str, Any]) -> tuple[list[dict[str, Any]], list[dict[str, Any]]]:
    ...
    for index, record in enumerate(registry.get("strategies", [])):
        ...
        if candidate_id == STRATEGY_ID:
            ...
            aliases.append(row)
        elif score >= 8 and candidate_id != STRATEGY_ID:
...
def load_authoritative_inputs() -> dict[str, Any]:
    ...
    exploration_outcome = next(
        row for row in read_csv_rows(EXPLORATION_DIR / "outcome_summary.csv") if row.get("strategy_id") == STRATEGY_ID
    )
    return {
        "blocked_closure_manifest": read_yaml(BLOCKED_CLOSURE_DIR / "closure_manifest.yaml"),
        "blocked_closure_consistency": read_json(BLOCKED_CLOSURE_DIR / "consistency_check.json"),
...
def evidence_gate(inputs: dict[str, Any]) -> tuple[bool, list[str]]:
    blockers: list[str] = []
    if inputs["blocked_closure_consistency"].get("process_failure_reason") != "status_reconciliation_required":
        blockers.append("prior_closure_blocker_not_status_reconciliation_required")
    ...
    if inputs["exploration_outcome"].get("outcome") != "exploratory_followup_candidate_diversifier":
    ...
    return [
        {
            ...
            "change_description": "hrp_authoritative_closed_record_reconciled"
...
def outcome_row(
    ...
    return {
        ...
        "total_exact_hrp_records_after_reconciliation": exact_after,
...
def consistency_payload(
    ...
    payload = {
        ...
        "closure_scope_exact_configuration_only": bool(
            final_record
            and final_record.get("closure_scope") == "exact_five_etf_252d_sample_cov_single_linkage_monthly_next_session_close_configuration_only"
        ),
        **FORBIDDEN_FLAGS,
    }
...
def report_text(
    ...
    lines = [
        "# Reconcile Missing HRP Registry Record And Close V1",
        "",
        ...
        f"Total exact HRP records after reconciliation: `{exact_after}`",
    ...
    if process_failure_reason:
        ...
        lines.extend(
            [
                "The exact tested HRP configuration was recorded directly as closed.",
    ...
    lines.extend(
        [
            ...
            "",
            "The closure is limited to SPY|EEM|IEF|DBC|VNQ with 252-day daily log returns, sample covariance, single linkage, lexical tie handling, monthly rebalance, and next-session-close execution.",
            "",
...
FIELDS = {
    ...
    "outcome_summary.csv": [
        ...
        "total_exact_hrp_records_after_reconciliation",
...
def run() -> dict[str, Any]:
    ...
    manifest = {
        ...
        "authoritative_strategy_records_created": created,
        "authoritative_strategy_records_updated": updated,
        "total_exact_hrp_records_after_reconciliation": exact_after_count,
        "existing_experiment_trials_carried_forward": 2,
        "new_experiment_trials": 0,
