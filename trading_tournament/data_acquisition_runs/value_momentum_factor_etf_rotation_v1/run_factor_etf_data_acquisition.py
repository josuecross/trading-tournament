...
APPROVED_SYMBOLS = ("MTUM", "VLUE", "VTV", "QUAL", "USMV", "SPLV")
...
def validate_requested_symbols(config: dict[str, Any], requested_symbols: list[str] | None = None) -> list[str]:
    ...
    excluded = {str(symbol).upper() for symbol in config.get("excluded_default_refresh_symbols", [])}
    ...
    excluded_requested = sorted(set(symbols) & excluded)
    if excluded_requested and not allow_refresh:
        raise ValueError(f"excluded refresh symbols requested without approval: {', '.join(excluded_requested)}")
    return symbols


def package_versions() -> dict[str, str]:
    packages = ["pandas", "numpy", "yfinance", "PyYAML"]
...
def analyze_symbol(
    ...
    raw_standard: pd.DataFrame | None = None
    ...
    spy_dates = cached_date_set(cache_root, "SPY")
    bil_dates = cached_date_set(cache_root, "BIL")
    common = symbol_dates & spy_dates & bil_dates
    common_overlap = ""
    common_count = 0
    if common:
    ...
    if error or not downloaded or not cache_written or not adjusted_constructible or not enough_180 or missing_adj or missing_close or duplicate_date_count:
        ...
        quality_status = "pass"
    notes = error or ("pass" if quality_status == "pass" else "warning: missing volume values")
...
def write_report_files(
    ...
    if not coverage.empty and coverage["first_date"].astype(str).ne("").all() and coverage["last_date"].astype(str).ne("").all():
        common_start = str(pd.to_datetime(coverage["first_date"]).max().date())
        common_end = str(pd.to_datetime(coverage["last_date"]).min().date())

    readme = f"""# Factor ETF Data Acquisition Packet

This is a data acquisition and quality packet only for `{SUBJECT_ID}`.

No strategy was implemented. No backtest was run. No paper-forward rule was changed. No broker integration, live orders, or real-money recommendation were added.
...
Future strategy implementation may be reviewed only after data quality is accepted. This packet does not implement or validate a strategy.

No real-money recommendation.
"""
    warnings = """# Warnings And Limitations

- yfinance/Yahoo data can have revisions, gaps, licensing/personal-use limits, and adjustment differences.
...
- No strategy implementation, backtest, broker integration, live orders, or real-money recommendation is included.
"""
...
def update_registry_status(repo_root: Path, status: str, next_action: str, summary: str) -> None:
    ...
    for row in data.get("strategies", []):
        if row.get("id") == SUBJECT_ID:
            ...
            row["evidence_source"] = "factor_etf_data_acquisition"
            row["latest_evidence_path"] = "evidence/data_acquisition_runs/value_momentum_factor_etf_rotation_v1/latest/"
            row["latest_known_result_summary"] = summary
            row["allowed_next_action"] = next_action
            forbidden = list(row.get("forbidden_next_actions", []))
            for action in [
                "implement_strategy_without_quality_review",
            ...
            row["promotion_blockers"] = "strategy_not_implemented;implementation_review_update_required;not_paper_forward_allowed"
            row["promotion_requirements"] = "Update implementation review after data quality, define fixed rule prompt, run research_sample evidence, and complete separate promotion review."
            break
...
def run_acquisition(
    ...
    if quality_counts["fail"] == 0 and quality_counts["warning"] == 0:
        strategy_status = "data_quality_review_passed"
        next_action = "update_implementation_review_after_data_quality"
    elif quality_counts["fail"] == 0:
        strategy_status = "data_acquired_pending_quality_check"
    ...
    metadata = {
        ...
        "yfinance_version": package_versions().get("yfinance", "not_installed"),
    ...
    manifest = {
        ...
        "live_orders": False,
        "real_money_recommendation": False,
        "latest_folder_file_count": latest_file_count,
    ...
    if update_registry:
        summary = (
            f"Factor ETF data acquisition run {run_id}; quality counts "
            f"pass={quality_counts['pass']}, warning={quality_counts['warning']}, fail={quality_counts['fail']}; "
            "strategy remains not implemented."
        )
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Acquire approved factor ETF proxy data only.")
