...
BASELINES = ["SPY", "QQQ", "BIL", "GLD", "IEF"]
CONDITIONAL_BENCHMARKS = ["TLT", "AGG"]
APPROVED_SYMBOLS = WRAPPERS + BASELINES + CONDITIONAL_BENCHMARKS
NEXT_LANE_ACTION = "create_dual_momentum_paa_etf_wrapper_fast_exploration_review_prompt"
PROMOTION_ACTION = "create_promotion_review_for_managed_futures_etf_wrapper"
...
def state_is_ready(root: Path, registry: dict[str, Any]) -> tuple[bool, list[str]]:
    ...
    if not review_manifest.exists():
        mismatches.append("managed futures review manifest missing")
    else:
        ...
        if payload.get("lane_verdict") != "approve_future_research_sample_prompt":
            mismatches.append("managed futures review gate did not approve future research_sample")
        if payload.get("next_action") != "create_managed_futures_etf_wrapper_research_sample_prompt":
            mismatches.append("managed futures review next action is not research_sample prompt")
    rows = rows_by_id(registry)
    managed = rows.get(FAMILY_ID, {})
    if managed.get("priority_rank") != 1:
        mismatches.append("managed futures ETF wrapper is not priority 1")
...
def qa_cache(root: Path, symbol: str) -> dict[str, Any]:
    ...
    reason = "" if include else "missing, duplicate, non-positive, or insufficient adjusted close history"
...
def prepared(close: pd.DataFrame) -> dict[str, Any]:
    ...
    sma200 = close.rolling(200).mean().to_numpy(dtype=float)
    above = values > sma200
    trailing126 = (close / close.shift(126) - 1.0).replace([np.inf, -np.inf], np.nan).fillna(-1e9).to_numpy(dtype=float)
    returns = np.zeros_like(values)
    returns[1:] = values[1:] / values[:-1] - 1.0
    vol60 = pd.DataFrame(returns, index=close.index, columns=close.columns).rolling(60).std().replace(0, np.nan).fillna(np.nan).to_numpy(dtype=float)
    months = np.array([dt.year * 12 + dt.month for dt in close.index], dtype=int)
    return {"close": close, "symbols": list(close.columns), "idx": {sym: i for i, sym in enumerate(close.columns)}, "above": above, "ret126": trailing126, "returns": returns, "vol60": vol60, "months": months}


def select_best_wrapper(p: dict[str, Any], t: int, available: list[str], risk_adjusted: bool = False) -> list[str]:
    ...
    return [sym for sym, _ in sorted(picks, key=lambda item: item[1], reverse=True)]


def weights_for_variant(p: dict[str, Any], t: int, variant: str, available_wrappers: list[str]) -> dict[str, float]:
    weights: dict[str, float] = {}
    if variant == "mf_wrapper_top1_trend_v1":
        picks = select_best_wrapper(p, t, available_wrappers)
        weights[picks[0] if picks else "BIL"] = 1.0
    elif variant == "mf_wrapper_top2_risk_adjusted_v1":
        picks = select_best_wrapper(p, t, available_wrappers, risk_adjusted=True)[:2]
        for pick in picks:
            weights[pick] = weights.get(pick, 0.0) + 0.5
        if len(picks) < 2:
            weights["BIL"] = weights.get("BIL", 0.0) + (2 - len(picks)) * 0.5
    elif variant == "mf_wrapper_plus_spy_70_30_v1":
        spy = p["idx"]["SPY"]
        weights["SPY" if p["above"][t, spy] else "BIL"] = weights.get("SPY" if p["above"][t, spy] else "BIL", 0.0) + 0.7
        picks = select_best_wrapper(p, t, available_wrappers)
        weights[picks[0] if picks else "BIL"] = weights.get(picks[0] if picks else "BIL", 0.0) + 0.3
    elif variant == "mf_wrapper_defensive_cash_switch_v1":
        picks = select_best_wrapper(p, t, available_wrappers)
        if len(picks) >= 2:
            for pick in picks:
                weights[pick] = 1.0 / len(picks)
        elif len(picks) == 1:
            weights[picks[0]] = 0.5
            weights["BIL"] = 0.5
        else:
...
def benchmark_weights(p: dict[str, Any], t: int, benchmark: str) -> dict[str, float]:
    if benchmark == "SPY_200d":
        spy = p["idx"]["SPY"]
        return {"SPY": 1.0} if p["above"][t, spy] else {"BIL": 1.0}
...
def simulate(
    ...
    for offset in range(1, horizon + 1):
        ...
        if month != last_month:
            new_weights = benchmark_weights(p, signal, benchmark) if benchmark else weights_for_variant(p, signal, variant or "", available_wrappers)
...
def sample_starts(length: int, horizon: int) -> list[int]:
    ...
    starts = list(range(252, max_start + 1))
...
def summarize_windows(results: pd.DataFrame, variant: str, horizon: int) -> dict[str, Any]:
    ...
    return {
        ...
        "max_drawdown_median": float(subset["max_drawdown"].median()),
        ...
        "profit_to_median_drawdown_ratio": abs(median_profit / float(subset["max_drawdown"].median())) if float(subset["max_drawdown"].median()) else "",
    }


def full_period_returns(p: dict[str, Any], variant: str | None, available_wrappers: list[str], benchmark: str | None = None) -> pd.Series:
    equity = STARTING_EQUITY
    weights: dict[str, float] = {}
    last_month = None
    values = []
    for today in range(253, len(p["close"])):
        ...
        month = int(p["months"][today])
        if month != last_month:
            weights = benchmark_weights(p, signal, benchmark) if benchmark else weights_for_variant(p, signal, variant or "", available_wrappers)
            last_month = month
        equity *= 1.0 + sum(weight * float(p["returns"][today, p["idx"][sym]]) for sym, weight in weights.items() if sym in p["idx"])
        values.append(equity)
    series = pd.Series(values, index=p["close"].index[253:])
    return series.pct_change().dropna()


def score_and_verdict(summary: dict[str, Any], overlap: dict[str, Any], severe_short: bool, missing_count: int, evidence_missing: bool) -> tuple[float, str, str]:
    if evidence_missing or summary.get("validation_status") != "complete":
    ...
    practicality = 10.0 - (5.0 if severe_short else 0.0) - min(5.0, missing_count * 1.0)
    score = max(0.0, profit + risk + additive + practicality)
    if severe_short:
        score -= 25
    if missing_count:
        score -= 20
    ...
    elif score >= 70 and float(summary["target_300_before_stop_rate"]) >= 0.25 and not severe_short:
...
def update_registry(root: Path, registry: dict[str, Any], verdicts: list[dict[str, Any]], family_verdict: str, next_action: str, output_dir: str) -> dict[str, Any]:
    ...
    for verdict in verdicts:
        ...
        row["latest_known_result_summary"] = (
            f"Fast exploratory managed-futures ETF-wrapper research_sample verdict: {verdict['strategy_verdict']}. "
            "Exploratory non-final evidence only."
        )
        ...
        row["primary_failure_mode"] = "duplicate_or_speed_or_evidence_gate"
...
def run_research_sample(root: Path = ROOT, downloader: Downloader | None = None, allow_download: bool = True, update_registry_file: bool = True) -> dict[str, Any]:
    ...
    available_wrappers = [symbol for symbol in WRAPPERS if symbol in close.columns]
    severe_short = len(available_wrappers) < 2 or len(close) < 432 or any(row["short_history_warning"] for row in qa_rows if row["symbol"] in available_wrappers)
    ...
    if close.empty or len(close) < 432 or "BIL" not in close or not available_wrappers:
        for variant in VARIANT_IDS:
            verdict_rows.append({"strategy_id": variant, "strategy_verdict": "evidence_missing", "reason": "insufficient common adjusted close history or no wrapper symbols available", "next_allowed_action": NEXT_LANE_ACTION})
        ...
        next_action = "create_managed_futures_etf_wrapper_symbol_discovery_prompt"
    else:
        p = prepared(close)
        simulated_variants = [variant for variant in VARIANT_IDS if variant != "mf_wrapper_plus_dsr_vm_combo_proxy_v1"]
        for variant in simulated_variants:
            for horizon in HORIZONS:
                for start in sample_starts(len(close), horizon):
                    row = simulate(p, start, horizon, variant, available_wrappers)
        ...
        for variant in simulated_variants:
            for horizon in HORIZONS:
                rolling_summary.append(summarize_windows(window_frame, variant, horizon))
        bench_ids = ["SPY_200d"] + [f"{symbol}_buy_hold" for symbol in ["SPY", "QQQ", "GLD", "BIL", "IEF", "TLT", "AGG", *available_wrappers] if symbol in close.columns]
        for variant in simulated_variants:
            variant_returns = full_period_returns(p, variant, available_wrappers)
            corr_values: dict[str, float] = {}
            for benchmark in bench_ids:
                bench_returns = full_period_returns(p, None, available_wrappers, benchmark=benchmark)
                aligned = pd.concat([variant_returns.rename("variant"), bench_returns.rename("benchmark")], axis=1).dropna()
                corr = float(aligned["variant"].corr(aligned["benchmark"])) if len(aligned) > 5 else np.nan
                corr_values[benchmark] = corr
            ...
            additive_label = "possible_additive" if max_corr < 0.70 else "weak_or_unclear_additive"
            duplicate_rows.append({
                ...
                "short_history_warning": severe_short,
            ...
            score, label, verdict = score_and_verdict(summary180, duplicate_rows[-1], severe_short, len(missing_symbols), False)
            score_rows.append({"strategy_id": variant, "profit_first_score": round(score, 2), "score_label": label, "strategy_verdict": verdict})
            verdict_rows.append({"strategy_id": variant, "strategy_verdict": verdict, "reason": f"score={score:.2f}; duplicate={duplicate_label}; short_history={severe_short}", "next_allowed_action": PROMOTION_ACTION if verdict == "promotion_review_candidate" else NEXT_LANE_ACTION})
            variant_results.append({**summary180, "profit_opportunity_score": round(score, 2), "strategy_verdict": verdict, "short_history_warning": severe_short})
            for benchmark in bench_ids:
                bench_results = [simulate(p, start, 180, None, available_wrappers, benchmark=benchmark) for start in sample_starts(len(close), 180)]
        ...
        verdict_rows.append({"strategy_id": "mf_wrapper_plus_dsr_vm_combo_proxy_v1", "strategy_verdict": "evidence_missing", "reason": "protected active combo/VM/DSR benchmark series not safely inferable without mutation", "next_allowed_action": NEXT_LANE_ACTION})
        score_rows.append({"strategy_id": "mf_wrapper_plus_dsr_vm_combo_proxy_v1", "profit_first_score": 0, "score_label": "reject", "strategy_verdict": "evidence_missing"})
        duplicate_rows.append({"strategy_id": "mf_wrapper_plus_dsr_vm_combo_proxy_v1", "correlation_with_active_combo": "unavailable", "correlation_with_vm_quality": "unavailable", "correlation_with_dsr_equal_weight": "unavailable", "duplicate_risk_label": "evidence_missing", "additive_value_label": "evidence_missing", "short_history_warning": True, "max_abs_available_correlation": ""})
        ...
        elif len(available_wrappers) < 2 or len(close) < 756:
            family_verdict = "needs_more_data"
    ...
    updated_registry = update_registry(root, registry, verdict_rows, family_verdict, next_action, str(output_dir))
    ...
    manifest = {
        ...
        "available_wrappers": available_wrappers,
        "downloaded_symbols": downloaded,
        "provider_api_called": provider_api_called,
        "data_downloaded": bool(downloaded),
        "family_verdict": family_verdict,
    ...
    write_csv(output_dir / f"{FAMILY_ID}_failure_modes.csv", [{"failure_mode": "short_history_or_missing_wrapper", "observed": severe_short}, {"failure_mode": "protected_proxy_evidence_missing", "observed": True}], ["failure_mode", "observed"])
    (output_dir / f"{FAMILY_ID}_short_history_review.md").write_text(f"# Short History Review\n\nShort history warning: `{severe_short}`.\nAvailable wrappers: `{', '.join(available_wrappers) or 'none'}`.\nMissing symbols: `{', '.join(missing_symbols) or 'none'}`.\n", encoding="utf-8")
    ...
    (output_dir / f"{FAMILY_ID}_research_sample_summary.md").write_text(f"# Managed Futures ETF Wrapper Research Sample\n\nExploratory non-final research_sample for ETF/fund wrappers only.\n\nFamily verdict: `{family_verdict}`\n\nNext action: `{next_action}`\n\nDownloaded symbols: `{', '.join(downloaded) or 'none'}`\n\nAvailable wrappers: `{', '.join(available_wrappers) or 'none'}`\n", encoding="utf-8")
...
def main() -> int:
    ...
    print(f"managed_futures_research_sample_latest_dir={result['output_dir']}")
