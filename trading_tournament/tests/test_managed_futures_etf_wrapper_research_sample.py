...
import run_managed_futures_etf_wrapper_research_sample as sample
...
def write_state(root: Path) -> None:
    ...
    rows = [
        ...
        base_row(sample.FAMILY_ID, sample.FAMILY_ID, "research_sample_candidate", "create_managed_futures_etf_wrapper_research_sample_prompt", False, 1),
    ]
...
def seed_data(root: Path, wrapper_periods: int = 620) -> None:
    for symbol in sample.BASELINES + sample.CONDITIONAL_BENCHMARKS:
        write_cache(root, symbol, 700, 0.00025)
    write_cache(root, "DBMF", wrapper_periods, 0.00035)
    write_cache(root, "KMLM", wrapper_periods, 0.00015)
...
def test_fixed_variants_and_no_forbidden_flags(tmp_path: Path) -> None:
    ...
    variants = pd.read_csv(latest / "managed_futures_etf_wrapper_strategy_verdicts.csv")
    assert set(sample.VARIANT_IDS) == set(variants["strategy_id"])
...
def test_missing_symbols_and_short_history_are_recorded(tmp_path: Path) -> None:
    write_state(tmp_path)
    seed_data(tmp_path, wrapper_periods=320)
    result = sample.run_research_sample(tmp_path, allow_download=False)
    latest = Path(result["output_dir"])
    missing = pd.read_csv(latest / "managed_futures_etf_wrapper_missing_symbols.csv")
    assert {"CTA", "FMF", "WTMF"}.issubset(set(missing["symbol"]))
    short_text = (latest / "managed_futures_etf_wrapper_short_history_review.md").read_text(encoding="utf-8")
    assert "Short history warning" in short_text
    assert result["family_verdict"] in {"needs_more_data", "watchlist_family", "promotion_review_candidate_family"}


def test_active_observations_unchanged_and_backlog_rows_safe(tmp_path: Path) -> None:
    write_state(tmp_path)
    seed_data(tmp_path)
    ...
    for row_id, row in after_rows.items():
        if row_id.startswith("mf_wrapper") or row_id == sample.FAMILY_ID:
