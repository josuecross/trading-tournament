...
def write_required_cache(root: Path) -> None:
    for offset, symbol in enumerate(batch2.required_symbols()):
        if symbol == "BIL":
...
def write_symbol_map(root: Path) -> None:
    rows = [{"symbol": symbol, "allowed_for_strategy": True, "allowed_for_benchmark": True} for symbol in sorted(batch2.required_symbols())]
...
def write_registry(root: Path) -> None:
    rows = [
        row(base.VM_ID, True, "active_paper_demo_observation", "paper_forward_activation_recovered"),
        row(base.DSR_ID, True, "active_paper_demo_observation", "paper_forward_activation_recovered"),
        row(base.SPY_200D_ID, True, "active_observation", "keep_frozen_control"),
        row(base.TOP2_ID, False, "mark_duplicate_or_near_duplicate", "mark_duplicate_or_near_duplicate"),
        row(base.TOP3_ID, False, "mark_duplicate_or_near_duplicate", "mark_duplicate_or_near_duplicate"),
        row("qvm_quality_value_momentum_risk_adjusted_top2_v1", False, "mark_too_risky", "mark_too_risky"),
        row("qvm_quality_value_momentum_top2_v1", False, "mark_duplicate_or_near_duplicate", "mark_duplicate_or_near_duplicate"),
        row("lvq_lowvol_quality_spy_regime_v1", False, "keep_watchlist", "keep_watchlist"),
    ]
    path = root / batch2.REGISTRY_PATH
    path.parent.mkdir(parents=True, exist_ok=True)
...
def audited_fixture(tmp_path_factory: pytest.TempPathFactory) -> dict[str, object]:
    root = prepared_root(tmp_path_factory.mktemp("batch2"))
    result = batch2.run_batch_2(root, strict_state=True)
    return {"root": root, "result": result}


def test_only_approved_symbols_are_used() -> None:
    allowed = set(batch2.required_symbols())
    for spec in batch2.specs():
        assert set(spec["symbols"]) <= allowed


def test_forbidden_symbols_are_rejected() -> None:
    with pytest.raises(ValueError):
        batch2.validate_spec_symbols({"strategy_id": "bad", "symbols": ["SPY", "DBC"]}, {"SPY", "BIL"})


def test_fixed_rules_are_defined_before_running() -> None:
    assert len(batch2.specs()) == 10
    assert all(spec.get("rule") for spec in batch2.specs())
    assert {spec["strategy_id"] for spec in batch2.specs()} == {
        "gdb_growth_defensive_barbell_60_40_v1",
        "gdb_growth_defensive_barbell_70_30_v1",
        "gdb_qmtum_gold_bond_cash_v1",
        "matr_multi_asset_top3_with_cash_floor_v1",
        "matr_multi_asset_top2_defensive_fallback_v1",
        "matr_global_assets_absolute_momentum_cash_v1",
        "qldg_quality_lowvol_growth_50_30_20_v1",
        "qldg_quality_lowvol_growth_equal_risk_proxy_v1",
        "benchmark_spy_qqq_gold_bond_equal_weight_v1",
        "benchmark_quality_lowvol_equal_weight_v1",
    }
...
def test_no_paper_forward_active_flag_is_set(audited_fixture: dict[str, object]) -> None:
    registry = yaml.safe_load((audited_fixture["root"] / batch2.REGISTRY_PATH).read_text(encoding="utf-8"))
    ids = {spec["strategy_id"] for spec in batch2.specs()}
...
def test_promotion_candidates_are_separate_from_watchlist_rows(audited_fixture: dict[str, object]) -> None:
    ...
    promotions = pd.read_csv(output / "approved_cache_batch_2_promotion_candidates.csv")
    watchlist = pd.read_csv(output / "approved_cache_batch_2_watchlist.csv")
...
def test_unavailable_benchmark_deltas_are_not_zero_filled(audited_fixture: dict[str, object]) -> None:
    output = Path(audited_fixture["result"]["output_dir"])
    deltas = pd.read_csv(output / "approved_cache_batch_2_benchmark_delta.csv")
...
def test_risk_buffer_too_thin_prevents_promotion() -> None:
    verdict, reason = batch2.classify(
...
def test_next_action_is_explicit(audited_fixture: dict[str, object]) -> None:
    assert audited_fixture["result"]["next_action"]


def test_consistency_check_passes(audited_fixture: dict[str, object]) -> None:
    output = Path(audited_fixture["result"]["output_dir"])
    consistency = json.loads((output / "approved_cache_batch_2_consistency_check.json").read_text(encoding="utf-8"))
