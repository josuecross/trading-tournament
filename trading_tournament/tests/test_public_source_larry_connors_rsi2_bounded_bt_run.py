...
from strategy_lab.research_os.research.public_source_larry_connors_rsi2_bounded_bt_run import (
    ...
    primary_rsi2_targets,
)


ROOT = Path(__file__).resolve().parents[1]
EVIDENCE = ROOT / "evidence" / "research_recovery" / "public_source_larry_connors_rsi2_bounded_bt_run" / "latest"


def load_manifest() -> dict:
    return json.loads(
        (EVIDENCE / "public_source_larry_connors_rsi2_bounded_bt_run_manifest.json").read_text(encoding="utf-8")
    )


def load_consistency() -> dict:
    return json.loads(
        (EVIDENCE / "public_source_larry_connors_rsi2_bounded_bt_run_consistency_check.json").read_text(
            encoding="utf-8"
        )
    )
...
def test_manifest_exact_bounded_lane_run_contract() -> None:
    ...
    assert manifest["public_source_larry_connors_rsi2_bounded_bt_lane_run"] is True
    ...
    assert manifest["rsi_entry_threshold"] == 5.0
...
def test_row_results_have_expected_roles_labels_and_similarity_status() -> None:
    manifest = load_manifest()
    rows = load_rows()
    ...
    assert primary["indicator_formula_status"] == "project_rsi_rolling_gain_loss_sma200_sma5_from_local_adjusted_close"
    assert "rsi_period=2" in primary["indicator_parameters"]
    assert "rsi_entry_threshold=5" in primary["indicator_parameters"]
    assert "trend_sma_period=200" in primary["indicator_parameters"]
    assert "exit_sma_period=5" in primary["indicator_parameters"]
...
def test_signal_state_helpers_are_deterministic() -> None:
    index = pd.date_range("2024-01-01", periods=8, freq="D")
    indicators = pd.DataFrame(
        {
            "rsi_2": [50.0, 4.0, 6.0, 7.0, 8.0, 4.0, 8.0, 9.0],
            "spy_close": [100.0, 105.0, 107.0, 108.0, 110.0, 106.0, 111.0, 112.0],
            "sma_200": [101.0, 100.0, 100.0, 100.0, 100.0, 100.0, 100.0, 100.0],
            "sma_5": [99.0, 110.0, 110.0, 120.0, 109.0, 120.0, 109.0, 120.0],
        },
        index=index,
    )
    indicators["entry_signal"] = (indicators["spy_close"] > indicators["sma_200"]) & (indicators["rsi_2"] < 5.0)
    indicators["exit_signal"] = indicators["spy_close"] > indicators["sma_5"]

    primary = primary_rsi2_targets(indicators)
    ...
    assert primary.loc["2024-01-06"].to_dict() == {"SPY": 1.0, "BIL": 0.0}
...
def test_required_evidence_files_and_next_action() -> None:
    ...
    assert manifest["next_action"] == "audit_public_source_larry_connors_rsi2_bounded_bt_results"
