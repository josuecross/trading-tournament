...
import run_managed_futures_etf_wrapper_review as review
...
def write_fixture(root: Path) -> None:
    ...
    (root / "strategy_lab" / "RESEARCH_ROADMAP.md").write_text(
    ...
    rows = [
        ...
        base_row("managed_futures_etf_wrapper", "managed_futures_etf_wrapper", "next_family_to_review", "create_managed_futures_etf_wrapper_fast_exploration_review_prompt", False, 1),
    ]
    payload = {
        "registry": {"schema_version": 1, "project": "trading_tournament", "research_only": True, "real_money_recommendation": False},
        "strategies": rows,
    }
    (root / "strategy_lab" / "strategy_registry.yaml").write_text(yaml.safe_dump(payload, sort_keys=False), encoding="utf-8")
...
def test_review_outputs_variants_and_boundaries_exist(tmp_path: Path) -> None:
    ...
    required = [
        "managed_futures_etf_wrapper_review.md",
        "managed_futures_etf_wrapper_family_thesis.md",
        "managed_futures_etf_wrapper_data_policy.md",
        "managed_futures_etf_wrapper_fixed_rules.md",
        "managed_futures_etf_wrapper_candidate_variants.csv",
        "managed_futures_etf_wrapper_risk_policy.md",
        "managed_futures_etf_wrapper_duplicate_risk_plan.md",
        "managed_futures_etf_wrapper_benchmark_plan.csv",
        "managed_futures_etf_wrapper_rejection_criteria.md",
        "managed_futures_etf_wrapper_next_action.md",
        "managed_futures_etf_wrapper_manifest.json",
        "managed_futures_etf_wrapper_consistency_check.json",
        "managed_futures_etf_wrapper_review_packet.zip",
    ]
    for name in required:
        assert (latest / name).exists()
    variants = pd.read_csv(latest / "managed_futures_etf_wrapper_candidate_variants.csv")
    assert set(variants["strategy_id"]) == set(result["approved_variants"])
    text = (latest / "managed_futures_etf_wrapper_data_policy.md").read_text(encoding="utf-8")
    assert "ETF/fund-wrapper" in text
    assert "direct futures contracts" in text


def test_next_action_and_no_run_flags_are_explicit(tmp_path: Path) -> None:
    write_fixture(tmp_path)
    result = review.run_review(tmp_path)
    latest = Path(result["output_dir"])
    manifest = json.loads((latest / "managed_futures_etf_wrapper_manifest.json").read_text(encoding="utf-8"))
    consistency = json.loads((latest / "managed_futures_etf_wrapper_consistency_check.json").read_text(encoding="utf-8"))
    assert manifest["next_action"] == "create_managed_futures_etf_wrapper_research_sample_prompt"
...
def test_active_observations_are_unchanged_and_registry_row_is_safe(tmp_path: Path) -> None:
    write_fixture(tmp_path)
    ...
    managed = after_rows["managed_futures_etf_wrapper"]
    assert managed["paper_forward_active"] is False
    assert managed["real_money_recommendation"] is False
    assert managed["candidate_exhaustive_run"] is False
    assert managed["allowed_next_action"] == "create_managed_futures_etf_wrapper_research_sample_prompt"
